...
def fetch_chesscom_games(username, max_games=100):
    ...
    if resp.status_code == 404:
        ...
        st.error(f"Failed to fetch archives (Status code {resp.status_code}).")
        return []

    archives = resp.json().get("archives", [])
    if not archives:
        st.warning(f"No match history found for '{username}'.")
        return []
...
with st.sidebar:
    ...
    game_limit = st.slider("Matches to Fetch", min_value=10, max_value=300, value=75, step=15)
...
if fetch_btn and username_input:
    with st.spinner(f"Pulling archive history for {username_input}..."):
